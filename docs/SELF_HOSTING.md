...
# 🖥️ Hébergement et exploitation

Faire tourner le bot en continu, le mettre à jour et le surveiller. Pré-requis et première installation sur [Installation](INSTALLATION.md).
...
```bash
cp .env.example .env && nano .env          # DISCORD_TOKEN + CLIENT_ID à changer minimum
docker compose up -d                       # lancer le bot seul
...
- La musique : dans `.env`, `LAVALINK_HOST=lavalink` et `LAVALINK_PASSWORD` (identique à `lavalink/application.yml`, voir [Lavalink](LAVALINK.md)).
- RSSHub : passe l'URL interne `http://rsshub:1200/...` à `/notif ajouter-rss`.
...
adduser --disabled-password unknown_variable
su - unknown_variable
git clone https://github.com/xeylou/unknown-variable bot_discord
cd bot_discord && npm install
cp .env.example .env && nano .env
npx prisma db push && npm run deploy
exit

cp /home/unknown_variable/unknown_variable/unknown_variable.service /etc/systemd/system/
systemctl daemon-reload
systemctl enable --now unknown_variable
systemctl status unknown_variable
```
...
|---|---|---|
| **Railway** | ~5 $·mois | Le plus simple : connecter le repo et ajouter les variables `.env`. |
...
⚠️ Les offres "free" s'endorment (Heroku free, Replit free, Glitch).

---

## Sonde de santé et monitoring

Le bot expose un endpoint HTTP **toujours actif** : `GET /health` -> `200 ok` (port `HEALTH_PORT`, défaut `3001`, `0` pour désactiver).

- Le `HEALTHCHECK` du Dockerfile l'interroge automatiquement -> `docker ps` affiche `healthy`/`unhealthy`.
- Brancher un moniteur externe (UptimeRobot, Healthchecks.io…) sur `http://<hôte>:3001/health`.
...
> ⚠️ Le `HEALTHCHECK` du Dockerfile cible le port `3001` en dur. Si changement de `HEALTH_PORT`, adapter le Dockerfile (ou le `docker-compose.yml`).
...
git pull
npm install                 # réinstaller si besoin + régénèrer le client Prisma (postinstall)
npx prisma db push          # uniquement si prisma/schema.prisma a changé - voir Base de données
systemctl restart unknown_variable
journalctl -u unknown_variable -f
```

**Avant un `git pull` :** sauvegarder la base (`cp data/<bot>.db data/backup-$(date +%F).db`) et la config du bot (`/backup export`). Détails : [Base de données](DATABASE.md).
...
```bash
npm outdated && npm update          # respecter le semver de package.json
...
- **Lavalink** : YouTube change ses protections -> relancer le plugin YouTube quand `/play` ne renvoie plus rien. Voir [Lavalink](LAVALINK.md).
...
| `The table main.X does not exist` au boot | `db push` a visé un autre fichier que le runtime - chemins désalignés (`BOT_NAME` · `DATABASE_PATH`). Relancer `npx prisma db push` puis `npm start`. |
| Aucune slash-command (jamais apparues) | Pas encore lancé `npm run deploy` |
...
| Le conteneur reste `unhealthy` | Vérifier que `HEALTH_PORT` n'est pas à `0` et correspond au port du `HEALTHCHECK` (3001). |
| Le bot ne crée pas les salons | Rôle du bot au-dessus du rôle Staff + permission `Manage Channels`. |
| `Missing Permissions` | Le bot n'a pas accès à la catégorie cible, ajuster ses permissions. |
| Ticket : "catégorie n'a pas de rôle responsable" | `/config ticket-role categorie:… role:…`. |
| Welcome card noire · vide | Vérifier les logs, désactiver avec `/config accueil carte-image:false`. |
...
Logs : `journalctl -u unknown_variable -f` (systemd) ou `docker compose logs -f bot` (Docker). Plus de détail BDD : [Base de données](DATABASE.md).
