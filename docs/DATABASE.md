...
> 🇬🇧 **English version → [DATABASE_en.md](DATABASE_en.md)** · ⬅️ [Retour au README](../README.md)
...
> ⚠️ `better-sqlite3` est **synchrone** : chaque requête bloque l'event loop. Parfait à charge modérée ; à grande échelle (sharding / multi-instance), envisage Postgres (voir plus bas).
...
Le repo **n'a pas de dossier `prisma/migrations/`** : les changements de schéma sont propagés avec **`prisma db push`** (compare `schema.prisma` à la base et applique le diff, sans fichier de migration). Adapté au mono-instance, et **idempotent**.
