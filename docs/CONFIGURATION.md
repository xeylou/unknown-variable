...
> 🇬🇧 **English version → [CONFIGURATION_en.md](CONFIGURATION_en.md)** · ⬅️ [Retour au README](../README.md)

Trois niveaux de configuration :
1. **Variables d'environnement** (`.env`) — secrets et options globales à l'hébergeur.
2. **Fichiers source** (`src/`) — branding et contenus éditables (redémarrage requis).
...
> Tout le reste se configure **par serveur** via `/config` — plus besoin de variables d'env par serveur. C'est ce qui rend le bot multi-serveur.

### Optionnelles

| Variable | Défaut | Description |
|---|---|---|
| `BOT_NAME` | `_unknown_variable` | Branding interne (logs, User-Agent, statut, **nom du fichier BDD**). Le nom **affiché dans Discord** vient du Developer Portal. |
...
| `STAFF_ROLE_ID` | *(vide)* | **Défaut du serveur principal uniquement.** Préfère `/config staff`. |
| `ADMIN_ROLE_ID` | *(vide)* | **Défaut du serveur principal uniquement.** Préfère `/config admin`. |
| `TICKET_CATEGORY_ID` | *(vide)* | **Défaut du serveur principal uniquement.** Préfère `/config tickets`. |
| `LOGS_CHANNEL_ID` | *(vide)* | **Défaut du serveur principal uniquement.** Salon des transcripts. Préfère `/config tickets`. |
| `DATABASE_PATH` | `./data/<slug BOT_NAME>.db` | Chemin du fichier SQLite. ⚠️ À fixer explicitement si tu changes `BOT_NAME` avec une base existante, sinon le bot pointera sur un nouveau fichier vide. |
...
| `TWITCH_CLIENT_ID` / `TWITCH_CLIENT_SECRET` | *(vide)* | Sans eux, `/notif ajouter-twitch` est désactivé. Création : <https://dev.twitch.tv/console/apps>. |
...
| `GITHUB_WEBHOOK_SECRET` | *(vide)* | Active le récepteur webhook (HMAC-SHA256) → temps réel. Sans token **ni** secret, le module GitHub est désactivé. |
| `GITHUB_WEBHOOK_PORT` / `_HOST` / `_PATH` | `3000` / `0.0.0.0` / `/github/webhook` | Écoute du récepteur webhook. |

> **GitHub — mode hybride** : renseigne `GITHUB_TOKEN` (polling, marche derrière un NAT) **et/ou** `GITHUB_WEBHOOK_SECRET` (webhooks temps réel). Détails : [§ Suivi GitHub](#suivi-github).

La validation des variables se fait au démarrage (schéma **Zod**) : une variable obligatoire manquante ou invalide **arrête le bot** avec un message listant précisément le problème.
...
**Catalogue des catégories de tickets** (`tickets.categories`) — structure/branding partagés par tous les serveurs :
...
> 🎯 Le **rôle responsable** d'une catégorie se définit par serveur (`/config ticket-role`), pas ici — c'est ce qui rend les tickets multi-serveur. Après modification du catalogue, **relance `/setup-tickets`**.

### Autres contenus éditables

| Fichier | Contenu |
|---|---|
| [`src/data/reglement.ts`](../src/data/reglement.ts) | Texte du règlement (en-tête, articles, acceptation, footer). Découpé auto en 2 embeds (limite 6000 car.). |
| [`src/data/help.ts`](../src/data/help.ts) | Catégories et signatures de `/help`, classement par tier. |
| [`src/data/welcome.ts`](../src/data/welcome.ts) | 2ᵉ embed d'orientation envoyé en MP à la vérification. |
...
Le bot est **bilingue** : l'**anglais est la langue canonique**, le français est servi selon `interaction.locale`. **Seules les descriptions et les réponses sont traduites — les noms de commandes/options restent inchangés.**
...
- Côté définition : `.setDescription(base(key))` + `.setDescriptionLocalizations(frLoc(key))`.
- Côté réponse : `const lang = resolveLang(interaction.locale)` puis `t(lang, key, vars)`.
- `resolveLang` mappe un client `fr*` → français, tout le reste → anglais.

> 🔧 **Déploiement progressif** : le socle est en place ; les commandes sont converties au fil de l'eau. Les non converties restent en français, sans rien casser. Voir [CONTRIBUTING](../CONTRIBUTING.md#i18n).
...
Tout ci-dessous est modifiable **sans redémarrage**, stocké en base (`guild_config`).
...
| `invitation` | `[url]` | URL d'invitation affichée dans les DM de kick/softban/unban. |
...
| Commande | Effet |
|---|---|
| `/setup-tickets` | Panneau du menu de tickets dans le salon courant. |
| `/setup-reglement` | Règlement (textes de `src/data/reglement.ts`) + bouton d'acceptation. |
...
> L'instance publique `rsshub.app` est rate-limitée ; pour de la prod, héberge ta propre instance (profil `rss` du [docker-compose](../docker-compose.yml)).
